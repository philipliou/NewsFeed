...
rss = SimpleRSS.parse open('http://www.businessweek.com/rss/bwdaily.rss')
...
# Store into DB
rss.items.each {|article| insertResultToDb(article)}
=begin
rss.items.each {|article| p "#{article.title}"
  p "Second url: #{article.guid}"
